...
#define BASE_H_INCLUDED
	...
	extern inline void FDrawVLine(Layer& id, unsigned int color, int x1, int y1, int k);
	void DrawHLine(Layer& id, unsigned int color, int x1, int y1, int k);
	extern inline void FDrawHLine(Layer& id, unsigned int color, int x1, int y1, int k);
	void DrawFrame(Layer& id, unsigned int color, int x, int y, int xk, int yk);
	extern inline void FDrawRect(Layer& id, unsigned int color, int x1, int y1, int xk, int yk);
	...
	extern inline void IncludeLayer(Layer& id, Layer& tid, int x1, int y1);
	void FragmentLayer(Layer& id, Layer& tid, int tx, int ty, int x2, int y2, int xk, int yk, int mode);
	extern inline void FragmentLayer(Layer& id, Layer& tid, int tx, int ty, int x2, int y2, int xk, int yk);
	extern inline void cloneLayer(Layer& id,Layer& tid);
